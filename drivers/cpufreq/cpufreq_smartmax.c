...
static void update_idle_time(bool online) {
	...
	for_each_possible_cpu(j)
	{
		...
		j_this_smartmax->prev_cpu_idle = get_cpu_idle_masa(j,
...
static int cpufreq_smartmax_boost_task(void *data) {
	...
	while (1) {
        ...
        this_smartmax->prev_cpu_idle = get_cpu_idle_masa(0,
		...
		for_each_online_cpu(cpu){
			...
			if (policy->cur < cur_boost_freq) {
				...
				this_smartmax->prev_cpu_idle = get_cpu_idle_masa(cpu, &this_smartmax->prev_cpu_wall);
			}
			mutex_unlock(&this_smartmax->timer_mutex);
